package io.github.gmathi.novellibrary.extension.en.purrfiction
...
class PurrFiction : HttpSource() {
    ...
    override fun novelDetailsParse(novel: Novel, response: Response): Novel {
        ...
        if (purrfictionGenres == null) {
            getPurrFictionGenres()
        }
    ...
    private fun getPurrFictionGenres() {
        try {
            ...
            val jsonArray = JsonParser.parseString(jsonString)?.asJsonArray
            purrfictionGenres = HashMap()
            jsonArray?.forEach {
                val genreObject = it.asJsonObject
                purrfictionGenres!![genreObject["id"].asInt] = genreObject["en"].asString
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private fun getPurrFictionTags() {
        try {
            ...
            val jsonArray = JsonParser.parseString(jsonString)?.asJsonArray
            purrfictionTags = HashMap()
            jsonArray?.forEach {
                val tagObject = it.asJsonObject
                purrfictionTags!![tagObject["id"].asInt] = tagObject["en"].asString
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private class PurrFictionChapter(
